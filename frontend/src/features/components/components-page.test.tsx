...
describe("Components routing and page", () => {
  ...
  it("renders every required Component field", async () => {
    installSuccessfulFetch()
    renderAppAt("/components")

    expect(await screen.findByText("curl")).toBeInTheDocument()
    expect(screen.getByText("8.14.1-r1")).toBeInTheDocument()
    ...
    expect(screen.getByText("haxx:curl")).toBeInTheDocument()
    expect(
      screen.getByText(componentFixture.cpe),
    ).toBeInTheDocument()
    expect(screen.getByText("a")).toBeInTheDocument()
    expect(
      screen.getByText("STRUCTURALLY_VALID"),
    ).toBeInTheDocument()
  })
  ...
  it("restores search, ordering, page, and page size from the URL", async () => {
    installSuccessfulFetch({ componentCount: 60 })
    renderAppAt(
      "/components?search=curl&ordering=-version&page=2&page_size=25",
    )
