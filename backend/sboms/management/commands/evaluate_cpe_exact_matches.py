...
class Command(BaseCommand):
    help = (
        "Evaluate raw Component.cpe strings against one COMPLETE "
        "CPE Dictionary snapshot without database writes."
    )
    ...
    def add_arguments(self, parser: CommandParser) -> None:
        ...
        parser.add_argument(
            ...
            help=(
                ...
                "analysis/results/cpe-exact-match/<snapshot-id>"
            ),
        )
        parser.add_argument(
            "--overwrite",
    ...
    def handle(self, *args, **options) -> None:
        try:
            ...
            analysis = build_cpe_exact_match_analysis(snapshot)
            output_directory = self.resolve_output_directory(
                options["output_dir"],
                snapshot.snapshot_id,
            )
            output_paths = write_cpe_exact_match_analysis(
                analysis,
                output_directory,
        ...
        self.stdout.write(
            self.style.SUCCESS(
                f"Wrote {len(output_paths)} exact-match files"
            )
        )
